...
import { invertTimeDirectionInFrames } from './utils-vertical';
...
export const TimeSeriesPanel = ({
  ...
  // Check if vertical time axis is enabled (custom option)
  // When enabled, we set the orientation to Vertical to rotate the chart 90 degrees
  const isVerticallyOriented = options.verticalTimeAxis || options.orientation === VizOrientation.Vertical;
  
  // Apply vertical orientation to options if verticalTimeAxis is enabled
  const effectiveOrientation = options.verticalTimeAxis ? VizOrientation.Vertical : options.orientation;
  const { frames, compareDiffMs } = useMemo(() => {
    let frames = prepareGraphableFields(data.series, config.theme2, timeRange);
    
    // Apply time direction inversion if enabled (only when vertical time axis is active)
    ...
    return { frames };
  }, [data.series, timeRange, options.verticalTimeAxis, options.invertTimeDirection]);
  ...
  // Prepare modified options with vertical orientation and time inversion
